...
- (id)init
{
    ...
    if (self) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] compare:@"8.0" options:NSNumericSearch] == NSOrderedDescending){
            BlockViewController *rootViewController = [[BlockViewController alloc] init];
...
- (void)removeView:(UIView *)view
{
    ...
    [self removeExtraViewWhenSystemUseGps];
...
/*解决系统有提示其他程序正在使用GPS时，切换到SFA程序此视图多出现了一个subView:(0, 20, self.width, self.height - 20)*/
- (void)removeExtraViewWhenSystemUseGps {
    CGRect extraViewFrame = CGRectMake(0, 20, self.width, self.height - 20);
    for (UIView *subView in self.subviews) {
        if (CGRectEqualToRect(subView.frame ,extraViewFrame)) {
...
# pragma mark - BlockViewController
