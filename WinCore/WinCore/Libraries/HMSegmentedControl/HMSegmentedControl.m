...
- (void)commonInit {
    ...
    self.bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0.0, self.frame.size.height - 1., CGRectGetWidth(self.bounds), 1)];
...
- (void)updateSegmentsRects {
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        for (NSString *titleString in self.sectionTitles) {
            ...
            CGFloat stringWidth = [titleString sizeWithFont:self.font].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
#endif
            stringWidth = stringWidth + 19.f; //定制每个segment的宽度 文字宽度+预设间隔
            totalWidth = totalWidth + stringWidth;
...
- (void)testAnimation
{
    CALayer*viewLayer= self.selectionIndicatorStripLayer;
