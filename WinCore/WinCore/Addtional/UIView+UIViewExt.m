...
@end
@implementation  UIView(UIViewAdditionsForScrollToTop)

//递归设置scrollsToTop属性，这个事应该是苹果做，但是苹果现在还没有做   sam wang .
-(void)disableScrollsToTopPropertyOnAllSubviews{
    for (UIView *subview in self.subviews) {
        if ([subview isKindOfClass:[UIScrollView class]] || [subview isMemberOfClass:[UIScrollView class]]) {
            if (((UIScrollView *)subview).scrollsToTop) {
...
+ (UIImageView *)borderStretchedImageViewWithOriginalImageName:(NSString *)imageName leftCapWidth:(NSInteger)leftCapWidth topCapHeight:(NSInteger)topCapHeight {
    ...
    UIImage *image = [UIImage imageFromMainBundleFile:imageName];
    
    if(image) {
        UIImage *stretchedImage = [image stretchableImageWithLeftCapWidth:leftCapWidth topCapHeight:topCapHeight];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:stretchedImage];
