//
//  WSContactsBookViewController.m
//  WinSFA
//
...
#import "WSContactsBookViewController.h"
...
#pragma mark - 通讯录视图管理器 延展(内部)
@interface WSContactsBookViewController ()
...
@property (nonatomic, assign) BOOL isDepartment;                            //是否存在部门标示
@property (nonatomic, assign) BOOL isOptStore;                              //是否存在门店标示
...
#pragma mark - 通讯录视图管理器 延展(工具)
@interface WSContactsBookViewController (Tools)

#pragma mark - 普通初始化方法
- (void)commonInit;

#pragma mark - 布局通讯录视图方法
- (void)layoutContactsBookView;

@end
//===================================================================================================================================================================

#pragma mark - 通讯录视图管理器 延展(实现UITableViewDelegate代理协议, 实现UITableViewDataSource数据源协议)
@interface WSContactsBookViewController (tableViewDelegateAndDataSource) <UITableViewDelegate, UITableViewDataSource>

@end
//===================================================================================================================================================================

#pragma mark - 通讯录视图管理器 延展(实现WSContactsBookHeaderViewDelegate代理协议)
@interface WSContactsBookViewController (contactsBookHeaderViewDelegate) <WSContactsBookHeaderViewDelegate>

@end
//===================================================================================================================================================================

#pragma mark - 通讯录视图管理器 延展(实现WSContactsBookSearchViewDelegate代理协议)
@interface WSContactsBookViewController (contactsBookSearchViewDelegate) <WSContactsBookSearchViewDelegate>

@end
//===================================================================================================================================================================

#pragma mark - 通讯录视图管理器
@implementation WSContactsBookViewController
...
#pragma mark - 通讯录视图管理器 延展(工具)
@implementation WSContactsBookViewController (Tools)
...
- (void)commonInit
{
    ...
    self.isOptStore = [self.currentFuncs.opt.cusMailList isEqualToString:@"1"];
    self.isDepartment = [WSContactsBookTools sharedManager].isDepartment;
    self.nameIndexArray = [[NSMutableArray alloc] initWithArray:[WSContactsBookTools sharedManager].nameIndexArray];
    self.nameSectionDictionary = [[NSMutableDictionary alloc] initWithDictionary:[WSContactsBookTools sharedManager].nameSectionDictionary];
    ...
    [self.headerView updateWithIsShowOrganize:self.isDepartment isShowStoreButton:self.isOptStore];
...
- (void)layoutContactsBookView
{
    ...
    CGFloat h = [self.headerView getHeightWithisShowOrganize:self.isDepartment isShowStoreButton:self.isOptStore maxWidth:CGRectGetWidth(self.view.frame)];
...
#pragma mark - 通讯录视图管理器 延展(实现UITableViewDelegate代理协议, 实现UITableViewDataSource数据源协议)
@implementation WSContactsBookViewController (tableViewDelegateAndDataSource)
...
#pragma mark - 通讯录视图管理器 延展(实现WSContactsBookHeaderViewDelegate代理协议)
@implementation WSContactsBookViewController (contactsBookHeaderViewDelegate)
...
{
    WSContactsBookDepartmentViewController *vc = [[WSContactsBookDepartmentViewController alloc] init];
    vc.title = NSLocalizedString(@"department", nil);
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
...
{
    WSStoreContactsBookViewController *vc = [[WSStoreContactsBookViewController alloc] init];
    vc.title = NSLocalizedString(@"customer_address_book", nil);
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
...
#pragma mark - 通讯录视图管理器 延展(实现WSContactsBookSearchViewDelegate代理协议)
@implementation WSContactsBookViewController (contactsBookSearchViewDelegate)
