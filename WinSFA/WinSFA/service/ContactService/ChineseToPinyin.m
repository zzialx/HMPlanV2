...
+ (NSString *)firstCharactorFromChiniseString:(NSString *)string {
    return [ChineseToPinyin firstCharactorFromChiniseString:string isUsingSystemMethod:NO];
}

+ (NSString *)firstCharactorFromChiniseString:(NSString *)string isUsingSystemMethod:(BOOL)isUsingSystemMethod {
    return [ChineseToPinyin firstCharactorFromChiniseString:string length:string.length isUsingSystemMethod:isUsingSystemMethod];
}

+ (NSString *)firstCharactorFromChiniseString:(NSString *)string length:(NSInteger)length isUsingSystemMethod:(BOOL)isUsingSystemMethod {
    ...
    for (NSInteger i = 0; i < length; i++) {
        ...
        if (isUsingSystemMethod) {
            ...
            // 可能会有一些生僻字无法转换
            str = [ChineseToPinyin pinyinFromChiniseString:subString];
        }
    
        if ([str length] > 0) {
            NSString *capString = [str capitalizedString];
            pinyin = [pinyin stringByAppendingString:[capString substringToIndex:1]];
        }
    }
    return pinyin;
}
...
+ (NSString *)getPinyinFromName:(NSString *)name {
    NSString *nameFullPinyin = [ChineseToPinyin pinyinFromChiniseString:name];
    NSString *nameFirstPinyin = [ChineseToPinyin firstCharactorFromChiniseString:name];
    NSString *pinyin = [NSString stringWithFormat:@"%@,%@", [nameFullPinyin lowercaseString], [nameFirstPinyin lowercaseString]];
    return pinyin;
}
@end
