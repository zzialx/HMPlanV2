...
@implementation HSWKWebViewJavascriptBridge
{
    __weak WKWebView* _webView;
    __weak id<WKNavigationDelegate> _webViewDelegate;
    long _uniqueId;
    HSWebViewJavascriptBridgeBase *_base;
}

/* API
 *****/

+ (void)enableLogging { [HSWebViewJavascriptBridgeBase enableLogging]; }

+ (instancetype)bridgeForWebView:(WKWebView*)webView {
    HSWKWebViewJavascriptBridge* bridge = [[self alloc] init];
...
- (void) _setupInstance:(WKWebView*)webView {
    ...
    _base = [[HSWebViewJavascriptBridgeBase alloc] init];
    _base.delegate = self;
}
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    [webView evaluateJavaScript:[_base webViewJavascriptCheckCommand]
              completionHandler:^(NSString *result, NSError *error) {
        if (![result boolValue]) {
            [self->_base injectJavascriptFile];
        }
