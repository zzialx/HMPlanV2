...
//  WinSFA
//
//  Created by donghong on 2018/3/12.
...
@class WSInputView;
@protocol WSInputViewDelagete <NSObject>
@optional
 ...
 */
-(void)xhInputViewWillShow:(WSInputView *)inputView;
...
/**
 ...
 @param inputView inputView
 */
-(void)xhInputViewWillHide:(WSInputView *)inputView;

@end

@interface WSInputView : UIView

@property (nonatomic, assign) id<WSInputViewDelagete> delegate;
 ...
 */
+(void)showWithStyle:(InputViewStyle)style configurationBlock:(void(^)(WSInputView *inputView))configurationBlock sendBlock:(BOOL(^)(NSString *text))sendBlock;
