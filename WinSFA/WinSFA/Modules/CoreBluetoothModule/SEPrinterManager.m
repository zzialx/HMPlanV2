...
+ (SEPrinterManager *)getSharedInstancePrinterManager {
    return instance;
}
...
- (void)resetBLEModel
{
  //  _centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:dispatch_get_main_queue()]; //这个如果没开启来呀 会有系统自带的弹出框
    NSDictionary *options = @{CBCentralManagerOptionShowPowerAlertKey:@NO};
   _centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:dispatch_get_main_queue() options:options];//这种方式是不需要系统的弹出框  在需要的地方自定义
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    if (central.state != CBCentralManagerStatePoweredOn) {
        ...
        if ([self.delegate respondsToSelector:@selector(showBlueToothListView)])
        {
            [self.delegate showBlueToothListView];
        }
        
    } else {
        self.isStatePoweredOn = YES;
        if ([self.delegate respondsToSelector:@selector(showBlueToothListView)])
        {
            [self.delegate showBlueToothListView];
        }
