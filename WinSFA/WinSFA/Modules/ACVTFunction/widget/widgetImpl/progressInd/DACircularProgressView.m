...
//        CGContextAddArcToPoint(context, 0, 0,2,2,10);
//
...
//         CGContextDrawImage(context, CGRectMake(centerPoint.x, centerPoint.y, 5, 5), _pointImage.CGImage);
...
+ (void) initialize
{
    if (self == [DACircularProgressView class]) {
        ...
        [circularProgressViewAppearance setProgressTintColor:[UIColor greenColor]];
...
//        [circularProgressViewAppearance setLineWidth:4.0];
...
{
    [self setProgress:progress animated:animated initialDelay:0.7];
}
       ...
       initialDelay:(CFTimeInterval)initialDelay
{
    ...
    CGFloat pinnedProgress = MIN(MAX(progress, 0.0f), max_percent);
    if (animated) {
        CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"progress"];
        animation.duration = initialDelay; // Same duration as UIProgressView animation
...
- (CGFloat)lineWidth{
    return self.circularProgressLayer.lineWidth;
}
