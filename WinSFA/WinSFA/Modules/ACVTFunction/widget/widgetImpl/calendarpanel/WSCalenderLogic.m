...
@interface WSCalenderLogic ()
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
    WSMonthCollectionViewCellModel *selectcalendarDay;
}

@end

@implementation WSCalenderLogic
...
- (NSMutableArray *)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    NSUInteger weeklyOrdinality = [firstDate weeklyOrdinality];//计算这个的第一天是礼拜几,并转为int型
    ...
    for (NSInteger i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        ...
        WSMonthCollectionViewCellModel *calendarDay = [WSMonthCollectionViewCellModel calendarDayWithYear:components.year month:components.month day:i];
        calendarDay.style = MonthCellDayTypeEmpty;//不显示
...
{
    NSDate *lastDayDate = [date lastDayOfCurrentMonth];
    NSUInteger weeklyOrdinality = [lastDayDate weeklyOrdinality];
    ...
    for (int i = 1; i < partialDaysCount + 1; ++i) {
        WSMonthCollectionViewCellModel *calendarDay = [WSMonthCollectionViewCellModel calendarDayWithYear:components.year month:components.month day:i];
        calendarDay.style = MonthCellDayTypeEmpty;
        [array addObject:calendarDay];
    }
}
...
- (void)changStyle:(WSMonthCollectionViewCellModel *)calendarDay
{
    ...
    if(calendarSelect.year == calendarDay.year &
       ...
       calendarSelect.day == calendarDay.day){
        ...
        calendarDay.style = MonthCellDayTypeClick;
        ...
        if (calendarToDay.year >= calendarDay.year &
            ...
            calendarDay.style = MonthCellDayTypePast;
            ...
            if (calendarDay.week == 1 || calendarDay.week == 7){
                calendarDay.style = MonthCellDayTypeWeek;
                
                //工作日
            }else{
                calendarDay.style = MonthCellDayTypeFutur;
            }
        }
    }
...
-(void)LunarForSolarYear:(WSMonthCollectionViewCellModel *)calendarDay{
...
- (void)selectLogic:(WSMonthCollectionViewCellModel *)day
{
    ...
    if (day.style == MonthCellDayTypeClick) {
        return;
    }
    
    day.style = MonthCellDayTypeClick;
    //周末
    if (selectcalendarDay.week == 1 || selectcalendarDay.week == 7){
        selectcalendarDay.style = MonthCellDayTypeWeek;
        
        //工作日
    }else{
        selectcalendarDay.style = MonthCellDayTypeFutur;
    }
