//
//  WSMonthCollectionViewCellModel.m
//  WinSFA
//
...
@implementation WSMonthCollectionViewCellModel


//公共的方法
+ (WSMonthCollectionViewCellModel *)calendarDayWithYear:(NSUInteger)year month:(NSUInteger)month day:(NSUInteger)day
{
    WSMonthCollectionViewCellModel *calendarDay = [[WSMonthCollectionViewCellModel alloc] init];//初始化自身
...
//判断是不是同一天
- (BOOL)isEqualTo:(WSMonthCollectionViewCellModel *)day
{
    BOOL isEqual = (self.year == day.year) && (self.month == day.month) && (self.day == day.day);
    return isEqual;
}

- (void)setDutyBean:(WSDutyBean *)dutyBean {
