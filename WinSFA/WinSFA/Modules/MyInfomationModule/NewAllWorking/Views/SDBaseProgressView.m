...
- (void)setCenterProgressText:(NSString *)text withAttributes:(NSDictionary *)attributes
{
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        ...
        if (attributes[NSFontAttributeName]) {
            strSize = [text ws_sizeWithFont:attributes[NSFontAttributeName] constrainedToWidth:CGFLOAT_MAX lineBreakMode:NSLineBreakByCharWrapping];
            attrStr = [[NSAttributedString alloc] initWithString:text attributes:attributes];
        } else {
            strSize = [text ws_sizeWithFont:[UIFont systemFontOfSize:[UIFont systemFontSize]] constrainedToWidth:CGFLOAT_MAX lineBreakMode:NSLineBreakByCharWrapping];
