...
- (void)initialization {
    ...
    _pointImage = [UIImage imageNamed:@"dot" inBundle:resourcesBundle compatibleWithTraitCollection:nil];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    //画小圆点
    if (_showPoint) {
        CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*cosf(valueEndA)-_strokeWidth*1.5, CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*sinf(valueEndA)-_strokeWidth*1.5, _strokeWidth*3, _strokeWidth*3), _pointImage.CGImage);
    }
