...
- (instancetype)initWithConversationChatter:(NSString *)conversationChatter
    ...
    if (self) {
        ...
        if(extDic){
            _conversation.ext=[NSDictionary dictionaryWithDictionary:extDic];
        }
...
- (void)_imageMessageCellSelected:(id<IMessageModel>)model
{
    ...
    }else if ([imageBody type] == EMMessageBodyTypeText){
        if (model.progress) {
            UIImage * image = [[SDImageCache sharedImageCache] imageFromKey:model.fileURLPath];
            [[EaseMessageReadManager defaultManager] showBrowserWithImages:@[image]];
        }else{
            //get the message thumbnail
            [weakSelf _reloadTableViewDataWithMessage:model.message];          
        ...
        dispatch_async(_messageQueue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (strongSelf) {
                    ...
                    [strongSelf.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[weakSelf.dataArray count] - scrollToIndex - 1 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:NO];
                }
            });
    ...
    [self.conversation loadMessagesStartFromId:messageId count:(int)count searchDirection:EMMessageSearchDirectionUp completion:^(NSArray *aMessages, EMError *aError) {
        ...
        if (!aError && [aMessages count]) {
            ...
            for (EMMessage *message in aMessages) {
                // SFA-17284 添加 weakSelf.store.Id 为空的判断
                if (!weakSelf.store.Id || [[weakSelf getstoreIDwithMsg:message] isEqualToString:weakSelf.store.Id]) {
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    EMMessageBodyType bodyType = [model getActualBodyTypeWith:model];
    switch (bodyType) {
...
- (void)didReceiveMessages:(NSArray *)aMessages
{
    for (EMMessage *message in aMessages) {
        if ([self.conversation.conversationId isEqualToString:message.conversationId]) {
            // 如果是当前门店的消息 才刷新页面
            NSString * storeId = [self getstoreIDwithMsg:message];
            if ([storeId isEqualToString:self.store.Id]) {
                ...
                {
                    if([self isMsgContanByStore:message]){
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WS_CHATNOTIFY_RESETUNREADNUMBER object:self.store.Id];
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    if(message.direction==EMMessageDirectionSend){
        //发送的消息
        if(message.ext){
            [message.ext setValuesForKeysWithDictionary:_conversation.ext];
        }else{
            message.ext =[NSDictionary dictionaryWithDictionary:_conversation.ext];
        }
    }
...
-(BOOL)isMsgContanByStore:(EMMessage*)msg{
    NSString * josnStr=[_conversation.ext objectForKey:WS_MSG_protyKey];
    NSDictionary * Dic=[josnStr objectFromJSONString];
    NSString*  storeID=[Dic objectForKey:WS_MSG_toStoreId];
    
    josnStr=[msg.ext objectForKey:WS_MSG_protyKey];
    Dic=[josnStr objectFromJSONString];
    NSString * msg_storeID=[Dic objectForKey:WS_MSG_toStoreId];
    
    if([storeID isEqualToString:msg_storeID]){
        return YES;
    }else{
        return NO;
    }
...
-(void)setMsgsAsReadState{
    NSArray * allMsgArray=[_conversation loadMoreMessagesFromId:nil limit:1000 direction:EMMessageSearchDirectionUp];
    for (EMMessage * msg in allMsgArray) {
        if(!msg.isRead){
            if([self isMsgContanByStore:msg]){
                
                [_conversation markMessageAsReadWithId:msg.messageId
                                                 error:nil];
            }
        }
    }
