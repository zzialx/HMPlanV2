...
CGFloat const EaseMessageCellPadding = 5;
...
+ (void)initialize
{
    ...
    cell.leftBubbleMargin = UIEdgeInsetsMake(2, 15, 2, 10);
    cell.rightBubbleMargin = UIEdgeInsetsMake(2, 10, 2, 15);
    cell.bubbleMargin = UIEdgeInsetsMake(2, 0, 2, 0);
                        ...
                        model:(id<IMessageModel>)model
{
    ...
    if (self) {
        _messageType = [model getActualBodyTypeWith:model];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if ([model.text hasPrefix:PHOTO_WALL_IMAGE_URL_PREFIX]) {
        model.fileURLPath = [[model.text componentsSeparatedByString:@";"] lastObject];
    }
                ...
                if (!image) {
                    ...
                    if (!image) {
                        if ([model.text hasPrefix:PHOTO_WALL_IMAGE_URL_PREFIX]) {
                            ...
                            [weakSelf.bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath]  placeholderImage:nil options:EMSDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                weakSelf.activity.hidden = NO;
                                [weakSelf.activity startAnimating];
                                weakSelf.model.progress = 0;
                                ...
                                if (!error) {
                                    ...
                                    weakSelf.model.progress = 1;
                                    weakSelf.activity.hidden = YES;
                                    
                                }else{
                                    [weakSelf showHint:NSLocalizedString(@"缩略图加载失败请点击重新加载", nil)];
                                }
...
- (void)bubbleViewTapAction:(UITapGestureRecognizer *)tapRecognizer
{
    if (tapRecognizer.state == UIGestureRecognizerStateEnded) {
        ...
        EMMessageBodyType bodyType = [_model getActualBodyTypeWith:_model];
        switch (bodyType) {
            case EMMessageBodyTypeImage:
            {
                if ([_delegate respondsToSelector:@selector(messageCellSelected:)]) {
