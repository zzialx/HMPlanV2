...
- (void)initialization
{
    ...
    _titleLabelAndFrontImageLineViewColor = [UIColor colorWithRed:255 green:255 blue:255 alpha:0.5];
...
- (void)setPageControlDotSize:(CGSize)pageControlDotSize
{
    ...
    } else if ([self.pageControl isKindOfClass:[WSPageControl class]]) {
        WSPageControl *pageControl = (WSPageControl *)_pageControl;
        pageControl.dotWidth = pageControlDotSize.width;
        pageControl.dotHeight = pageControlDotSize.height;
    }
}
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        ...
        case SDCycleScrollViewPageContolStyleClassic:
        {
            ...
            break;
        case SDCycleScrollViewPageContolStyleCustom:
        {
            WSPageControl *pageControl = [[WSPageControl alloc] init];
            pageControl.numberOfPages = self.imagePathsGroup.count;
            pageControl.currentPageIndicatorTintColor = self.currentPageDotColor;
            pageControl.pageIndicatorTintColor = self.pageDotColor;
            pageControl.userInteractionEnabled = NO;
            ...
            if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
                pageControl.pageControlAliment = WSPageContolAlimentRight;
            }
            [self addSubview:pageControl];
            _pageControl = pageControl;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage options:SDWebImageRetryFailed completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (error || !image) {
                    LogError("首页置顶图下载失败，%@,%@",imageURL, error);
                }
            }];
        } else {
    ...
    if (_frontImageNameGroup.count && itemIndex < _frontImageNameGroup.count) {
        cell.frontImageName = _frontImageNameGroup[itemIndex];
    }
    
    cell.isShowLine = self.isShowLine;
    
    if (!cell.hasConfigured) {
        cell.titleLabelAndFrontImageLineViewColor = self.titleLabelAndFrontImageLineViewColor;
