...
{
    [self removeGestureRecognizer:_movingGestureRecognizer];
}
...
- (void)moveRowToLocation:(CGPoint)location 
{
    ...
    if ([[self delegate] respondsToSelector:@selector(moveTableView:willMoveRowAtIndexPath:)]) {
        move= [[self delegate] moveTableView:self willMoveRowAtIndexPath:newIndexPath];
    }
    if(!move){
        return;
    }
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer
{
	switch ([gestureRecognizer state]) 
	{
		case UIGestureRecognizerStateBegan:
		{
            ...
            if ([[self delegate] respondsToSelector:@selector(moveTableView:willMoveRowAtIndexPath:)]) {
                self.canMove= [[self delegate] moveTableView:self willMoveRowAtIndexPath:touchedIndexPath];
            }
            if(!self.canMove){
                break;
            }
			...
			touchedCell.textLabel.text=nil;
            touchedCell.accessoryView=nil;
            touchedCell.imageView.image=nil;
		...
		case UIGestureRecognizerStateChanged:
		{
            ...
            UITableViewCell *firstCell =(UITableViewCell *)[cells firstObject];

            //NSLog(@"ddd%f",firstCell.frame.origin.y);//获取cell在table中的位置
            
            //NSLog(@"cellChanged x:%f y:%f",touchPoint.x,touchPoint.y);
            
            if (touchPoint.y < firstCell.frame.origin.y) {
                break;
            }
            ...
            // MMSH-2159 问题3 autoscroll 会导致无法调整位置错误
...
//									 [self reloadRowsAtIndexPaths:[NSArray arrayWithObject:movingIndexPath] withRowAnimation:UITableViewRowAnimationNone];
                                     [self reloadData];
                                     if (self.canMove) {
                                         [self.delegate didMoveUpdateSource];
                                     }
								 }
